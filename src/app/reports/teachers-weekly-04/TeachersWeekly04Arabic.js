...
export default function TeachersWeekly04Arabic() {
    const handlePrint = () => {
        window.print();
    };
    ...
    const getStatusBadge = (teacher) => {
        ...
        if (teacher.isPartial) {
            return <span className="status-badge status-partial" style={{ backgroundColor: '#f0ad4e', color: 'white' }}>⚠ جزئي</span>;
        }
        if (teacher.isExemplary) {
            return <span className="status-badge status-complete">⭐ نموذجي</span>;
        }
    ...
    return (
        <div className="teachers-report" dir="rtl">
            ...
            <div className="report-header">
                ...
                <span className="week-badge">📅 الأسبوع الرابع: {weeklyData.dateRange.ar}</span>
            </div>
                                        ...
                                        <div className="screenshot-gallery">
                                            <div className="screenshot-item">
                                                <div style={{ padding: '2rem', textAlign: 'center', color: '#666', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '8px', background: 'rgba(0,0,0,0.2)' }}>
                                                    {teacher.evidenceType === 'video' ? '🎬 فيديو' : teacher.evidenceType === 'audio' ? '🔊 صوت' : '🖼️ صورة'}
                                                </div>
                                                <div className="screenshot-caption">
                                                    {teacher.evidenceType === 'video' ? 'دليل فيديو' : teacher.evidenceType === 'audio' ? 'دليل صوتي' : 'دليل صورة'}
                                                </div>
                                            </div>
                                        </div>
                                    </>
